...
def main():
    ...
    model_dir = 'D:/Lucha_Data/final_networks/1pt8mm/Study1_SUBJ01_1pt8mm_VC_max_Stage3_20220913-233949_final.pth'
    ...
    for name, param in model.encoder.named_parameters():
        ...
        if param.requires_grad:
            ...
            print('Size: {}'.format(param.data.size()))
            data['fc1'].append(param.data)
        if count == 1:
            break

    import pandas as pd
    data_cpu = data['fc1'][0].cpu()
    data_np = data_cpu.numpy()
    data_mean = data_np.mean(0)
    # data_df = pd.DataFrame.from_dict(data)

            # data[name].append(param.data)
