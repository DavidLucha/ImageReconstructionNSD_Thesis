...
class Cog_Enc(nn.Module):
    ...
    def forward(self, x):
        ...
        batch_size=x.size()[0] # Doesn't do anything here.
...
PATH_S2 = 'CIFAR_VAEGAN_Stage_II.pt'
checkpoint_S2 = torch.load(PATH_S2)

PATH_S1 = 'CIFAR_VAEGAN_Stage_I.pt'
checkpoint_S1 = torch.load(PATH_S1)

# Load the model states
cog_gen.cog_enc.load_state_dict(checkpoint_S2['gen_cogenc_state_dict'])
cog_gen.decoder.load_state_dict(checkpoint_S1['gen_decoder_state_dict']) # STAGE 1 IMPORT
discrim.load_state_dict(checkpoint_S2['discrim_state_dict'])

# Load optimizer states
optimizer_cog_encoder.load_state_dict(checkpoint_S2['optimizer_cogenc_state_dict'])
optimizer_cog_decoder.load_state_dict(checkpoint_S1['optimizer_decoder_state_dict'])
optimizer_discriminator.load_state_dict(checkpoint_S2['optimizer_discriminator_state_dict'])
...
"""
# Init loaded
cog_gen.cog_enc.eval()
cog_gen.decoder.eval()
discrim.eval() 
...
# Set non-fixed models to train
cog_gen.cog_enc.eval()
cog_gen.decoder.train()
discrim.train()
...
# scheduler_CE = torch.optim.lr_scheduler.ExponentialLR(optimizer_cog_encoder, gamma=decay_rate) # Is fixed
scheduler_cog_de = torch.optim.lr_scheduler.ExponentialLR(optimizer_cog_decoder, gamma=decay_rate) 
...
for epoch in range(epochs):
  ...
  for i, (data,_) in enumerate(data_loader, 0):
    ...
    mean, logvar, rec_enc = cog_gen(datav) # Enc and Dec for Visual Encoder
    ...
    # Run the REAL (actual stimuli) through discrim
    ...
    optimizer_cog_decoder.zero_grad() # Set gradients to zero
    err_dec.backward(retain_graph=True) # Calculate derivative
    optimizer_cog_decoder.step() # Update the optimizer

    
    """ This optimizes encoder                                      <<<<<<<<<<<< EXPERIMENT """
    # Run the network again with updated decoder
    mean, logvar, rec_enc = cog_gen(datav) # Enc and Dec for Visual Encoder
