...
#SBATCH -N 1
#SBATCH --job-name=Lambda_GANComparison
#SBATCH -n 2
#SBATCH -c 25
#SBATCH --mem=20000
#SBATCH -o Lambda_GANComparison_output.txt
#SBATCH -e Lambda_GANComparison_error.txt
...
do
  ...
  do
    ...
    srun -N 1 -p gpu --gres=gpu:1 --mpi=pmi2 python /clusterdata/uqdlucha/scripts/deepReconPyTorch/train_stage2_WAE_iter.py --run_name WAE_Stage2_${SUBJ}_GANbda${LAMBDA}_${RUN_NAME}_${RUN_TIME} --load_from ${LOAD_FROM} --pretrained_net ${STAGE_1_NET} --load_epoch ${STAGE_1_EPOCH} --disc_loss Both --WAE_loss Both --lambda_WAE 1 --lambda_GAN ${LAMBDA} --batch_size ${BATCH_SIZE} --lr_enc 0.0001 --lr_disc 0.00005 --valid_shuffle True --latent_dims 512 --vox_res 1.8mm --set_size max --subject ${SUBJ} --ROI VC --num_workers 2 --epochs 40 --iters 15000 --dataset NSD --seed 277603 --message "Testing differarnat lambda_GAN values. This is at ${LAMBDA}"
