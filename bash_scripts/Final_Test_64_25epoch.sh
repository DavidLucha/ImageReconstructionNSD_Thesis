...
#SBATCH -N 1
#SBATCH --job-name=FINAL_TEST_bs64_ep25
#SBATCH -n 2
#SBATCH -c 25
#SBATCH --mem=20000
#SBATCH -o FINAL_TEST_bs64_ep25_output.txt
#SBATCH -e FINAL_TEST_bs64_ep25_error.txt
...
STAGE_1_EPOCH="249"
RUN_NAME="FINAL_TEST_bs64_ep25_SUBJ0${SUBJ}_${VOX_RES}_${ROI}_${SET_SIZE}"
MESSAGE="This is the default 64. But grabbing an early epoch from stage 2 (~10k iterations)."
# ------ CHANGE THESE ------- #
STAGE_2_NAME="FINAL_TEST_bs64_SUBJ01_1pt8mm_VC_max_Stage2_20220816-080152"
...
# srun -N 1 -p gpu --gres=gpu:1 --mpi=pmi2 python /clusterdata/uqdlucha/scripts/deepReconPyTorch/train_stage2_WAE_iter.py --run_name ${STAGE_2_NAME} --load_from ${LOAD_FROM} --pretrained_net ${STAGE_1_NET} --load_epoch ${STAGE_1_EPOCH} --standardize none --disc_loss Both --WAE_loss Both --lambda_WAE 1 --lambda_GAN 10 --lambda_recon 1 --batch_size 64 --lr_enc 0.001 --lr_disc 0.0005 --weight_decay 0.00001 --valid_shuffle True --latent_dims 1024 --lin_size 2048 --lin_layers 2 --clip_gradients False --vox_res ${VOX_RES} --set_size ${SET_SIZE} --subject ${SUBJ} --ROI ${ROI} --num_workers 2 --epochs 80 --iters 30000 --dataset NSD --seed 277603 --message "${MESSAGE}"
# echo "Stage 2 complete at $(date +%Y%m%d-%H%M%S)"

# Run stage 3
echo "Running stage 3 at $(date +%Y%m%d-%H%M%S)"
srun -N 1 -p gpu --gres=gpu:1 --mpi=pmi2 python /clusterdata/uqdlucha/scripts/deepReconPyTorch/train_stage3_WAE_iter.py --run_name ${STAGE_3_NAME} --load_from ${LOAD_FROM} --st1_net ${STAGE_1_NET} --st1_load_epoch ${STAGE_1_EPOCH} --st2_net ${STAGE_2_NAME} --st2_load_epoch 25 --standardize none --disc_loss Both --WAE_loss Maria --lambda_WAE 1 --lambda_GAN 10 --lambda_recon 1 --batch_size 64 --lr_dec 0.001 --lr_disc 0.0005 --valid_shuffle True --latent_dims 1024 --lin_size 2048 --lin_layers 2 --clip_gradients False --vox_res ${VOX_RES} --set_size ${SET_SIZE} --subject ${SUBJ} --ROI ${ROI} --num_workers 2 --epochs 40 --iters 15000 --dataset NSD --seed 277603 --message "${MESSAGE}"
