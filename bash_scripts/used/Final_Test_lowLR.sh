...
#SBATCH -N 1
#SBATCH --job-name=FINAL_TEST_lowLR
#SBATCH -n 2
#SBATCH -c 25
#SBATCH --mem=20000
#SBATCH -o FINAL_TEST_lowLR_output.txt
#SBATCH -e FINAL_TEST_lowLR_error.txt
...
STAGE_1_EPOCH="249"
RUN_NAME="FINAL_TEST_lowLR_SUBJ0${SUBJ}_${VOX_RES}_${ROI}_${SET_SIZE}"
...
srun -N 1 -p gpu --gres=gpu:1 --mpi=pmi2 python /clusterdata/uqdlucha/scripts/deepReconPyTorch/train_stage2_WAE_iter.py --run_name ${STAGE_2_NAME} --load_from ${LOAD_FROM} --pretrained_net ${STAGE_1_NET} --load_epoch ${STAGE_1_EPOCH} --standardize none --disc_loss Both --WAE_loss Both --lambda_WAE 1 --lambda_GAN 10 --lambda_recon 1 --batch_size 32 --lr_enc 0.0001 --lr_disc 0.0001 --weight_decay 0.00001 --valid_shuffle True --latent_dims 1024 --lin_size 2048 --lin_layers 2 --clip_gradients False --vox_res ${VOX_RES} --set_size ${SET_SIZE} --subject ${SUBJ} --ROI ${ROI} --num_workers 2 --epochs 40 --iters 30000 --dataset NSD --seed 277603 --message "${MESSAGE}"
