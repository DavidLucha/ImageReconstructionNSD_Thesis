...
#SBATCH -N 1
#SBATCH --job-name=optim_stage2_0003_shuffle
#SBATCH -n 1
#SBATCH -c 1
#SBATCH --mem=20000
#SBATCH -o optim_stage2_0003_shuffle_output.txt
#SBATCH -e optim_stage2_0003_shuffle_error.txt
...
srun -N 1 -p gpu --gres=gpu:1 --mpi=pmi2 python /clusterdata/uqdlucha/scripts/deepReconPyTorch/train_stage3_iter.py --run_name optim_stage_3_0003_noclip_subj0${SUBJ}_${RUN_NAME} --vox_res ${VOX_RES} --set_size ${SET_SIZE} --subject ${SUBJ} --stage_2_trained optim_stage_2_0003_shuffle_${RUN_NAME} --load_epoch final --lr_dec 0.00003 --lr_disc 0.00003 --valid_shuffle True --backprop_method clip --num_workers 2 --epochs 150 --iters 15000 --dataset NSD --seed 277603 --message "Stage 3, subject ${SUBJ}, at ${VOX_RES} voxel resolution, with set size of: ${SET_SIZE}"
