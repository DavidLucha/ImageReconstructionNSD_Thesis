...
#SBATCH -N 1
#SBATCH --job-name=Stage3_David_dubrecon
#SBATCH -n 2
#SBATCH -c 25
#SBATCH --mem=20000
#SBATCH -o Stage3_David_dubrecon_output.txt
#SBATCH -e Stage3_David_dubrecon_error.txt
...
# SUBJ=1
# ------ CHANGE THESE ------- #
RUN_NAME="Stage3_David_dubrecon"
VOX_RES="1.8mm"
BATCH_SIZE=64
SET_SIZE="max"
ROI="VC"
...
do
  ...
  srun -N 1 -p gpu --gres=gpu:1 --mpi=pmi2 python /clusterdata/uqdlucha/scripts/deepReconPyTorch/train_stage3_WAE_iter.py --run_name ${STAGE_3_NAME} --load_from ${LOAD_FROM} --st1_net ${STAGE_1_NET} --st1_load_epoch ${STAGE_1_EPOCH} --st2_net WAE_Stage2_2_Combo_dubrecon_20220812-170919 --st2_load_epoch final --standardize none --disc_loss David --WAE_loss Maria --lambda_WAE 1 --lambda_GAN 10 --lambda_recon 1 --batch_size ${BATCH_SIZE} --lr_dec 0.0001 --lr_disc 0.00005 --valid_shuffle True --latent_dims 1024 --lin_size 2048 --lin_layers 2 --vox_res 1.8mm --set_size max --subject ${SUBJ} --ROI VC --num_workers 2 --epochs 40 --iters 15000 --dataset NSD --seed 277603 --message "First double test using ideal david loss. Loading discrim from stage 1 and push cog latent to samp"
