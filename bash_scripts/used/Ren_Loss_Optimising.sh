...
echo "Running pretrain at $(date +%Y%m%d-%H%M%S)"
srun -N 1 -p gpu --gres=gpu:1 --mpi=pmi2 python /clusterdata/uqdlucha/scripts/deepReconPyTorch/pretrain.py --num_workers 2 --epochs 20 --dataset both --run_name 06_$RUN_NAME --gamma 0.75 --loss_method Ren --optim_method Adam --equilibrium_game n --message "gamma at 0.75, nw 2"
echo "Pretrain complete at $(date +%Y%m%d-%H%M%S)"

echo "Running pretrain at $(date +%Y%m%d-%H%M%S)"
srun -N 1 -p gpu --gres=gpu:1 --mpi=pmi2 python /clusterdata/uqdlucha/scripts/deepReconPyTorch/pretrain.py --num_workers 4 --epochs 20 --dataset both --run_name 08_$RUN_NAME --gamma 1.5 --loss_method Ren --optim_method Adam --equilibrium_game n --message "gamma at 1.5, nw 4"
echo "Pretrain complete at $(date +%Y%m%d-%H%M%S)"