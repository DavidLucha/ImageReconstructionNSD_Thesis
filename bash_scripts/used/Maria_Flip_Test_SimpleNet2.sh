...
MESSAGE="Trying Maria flip again but with the no momentum in the cogenc, and self init cogenc at bs16 and maria default enc lr. Reduce model complexity, even further, with 512 input and 512 latent."
# ------ CHANGE THESE ------- #
...
srun -N 1 -p gpu --gres=gpu:1 --mpi=pmi2 python /clusterdata/uqdlucha/scripts/deepReconPyTorch/train_stage2_WAE_iter.py --run_name ${STAGE_2_NAME} --load_from ${LOAD_FROM} --pretrained_net ${STAGE_1_NET} --load_epoch ${STAGE_1_EPOCH} --standardize none --disc_loss Maria_Flip --WAE_loss Maria --lambda_WAE 1 --lambda_GAN 10 --lambda_recon 1 --batch_size 16 --lr_enc 0.001 --lr_disc 0.0005 --valid_shuffle True --latent_dims 512 --lin_size 512 --lin_layers 1 --clip_gradients False --vox_res ${VOX_RES} --set_size ${SET_SIZE} --subject ${SUBJ} --ROI ${ROI} --num_workers 2 --epochs 130 --iters 100000 --dataset NSD --seed 277603 --message "${MESSAGE}"
echo "Stage 2 complete at $(date +%Y%m%d-%H%M%S)"

# Run stage 3
echo "Running stage 3 at $(date +%Y%m%d-%H%M%S)"
srun -N 1 -p gpu --gres=gpu:1 --mpi=pmi2 python /clusterdata/uqdlucha/scripts/deepReconPyTorch/train_stage3_WAE_iter.py --run_name ${STAGE_3_NAME} --load_from ${LOAD_FROM} --st1_net ${STAGE_1_NET} --st1_load_epoch ${STAGE_1_EPOCH} --st2_net ${STAGE_2_NAME} --st2_load_epoch final --standardize none --disc_loss Maria_Flip --WAE_loss Maria --lambda_WAE 1 --lambda_GAN 10 --lambda_recon 1 --batch_size 64 --lr_dec 0.001 --lr_disc 0.0005 --valid_shuffle True --latent_dims 512 --lin_size 1024 --lin_layers 1 --clip_gradients False --vox_res ${VOX_RES} --set_size ${SET_SIZE} --subject ${SUBJ} --ROI ${ROI} --num_workers 2 --epochs 160 --iters 60000 --dataset NSD --seed 277603 --message "${MESSAGE}"
