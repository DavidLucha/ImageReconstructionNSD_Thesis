...
#SBATCH -N 1
#SBATCH --job-name=tRUN_1.8max_subj01_st2n3
#SBATCH -n 1
#SBATCH -c 1
#SBATCH --mem=20000
#SBATCH -o tRUN_1.8max_subj01_st2n3_output.txt
#SBATCH -e tRUN_1.8max_subj01_st2n3_error.txt
...
srun -N 1 -p gpu --gres=gpu:1 --mpi=pmi2 python /clusterdata/uqdlucha/scripts/deepReconPyTorch/train_stage2_iter.py --run_name RUN_1.8max_stage2_subj0${SUBJ}_${RUN_NAME} --vox_res ${VOX_RES} --set_size ${SET_SIZE} --subject ${SUBJ} --pretrained_net ${PRE_NET} --load_from ${LOAD_FROM} --load_epoch ${LOAD_EP} --lr 0.00001 --backprop_method clip --num_workers 2 --epochs 150 --iters 30000 --dataset NSD --loss_method Maria --seed 277603 --message "Stage 2, subject ${SUBJ}, at ${VOX_RES} voxel resolution, with set size of: ${SET_SIZE}"
