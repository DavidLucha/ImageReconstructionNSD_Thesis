...
#SBATCH -N 1
#SBATCH --job-name=MSEloss_lowGANbda_0001t2
#SBATCH -n 2
#SBATCH -c 25
#SBATCH --mem=20000
#SBATCH -o MSEloss_lowGANbda_0001t2_output.txt
#SBATCH -e MSEloss_lowGANbda_0001t2_error.txt
...
# SUBJ=1
# ------ CHANGE THESE ------- #
RUN_NAME="MSEloss_lowGANbda_0001t2"
VOX_RES="1.8mm"
BATCH_SIZE=64
SET_SIZE="max"
ROI="VC"
...
do
  ...
  do
    ...
    srun -N 1 -p gpu --gres=gpu:1 --mpi=pmi2 python /clusterdata/uqdlucha/scripts/deepReconPyTorch/train_stage2_WAE_iter.py --run_name WAE_Stage2_${SUBJ}_${RUN_NAME}_${RUN_TIME} --load_from ${LOAD_FROM} --pretrained_net ${STAGE_1_NET} --load_epoch ${STAGE_1_EPOCH} --disc_loss Both --WAE_loss MSE --lambda_WAE 1 --lambda_GAN 2 --batch_size ${BATCH_SIZE} --lr_enc ${LR} --lr_disc 0.0001 --valid_shuffle True --latent_dims 512 --vox_res 1.8mm --set_size max --subject ${SUBJ} --ROI VC --num_workers 2 --epochs 40 --iters 15000 --dataset NSD --seed 277603 --message "Testing MSE loss, with lower lr and low gan lambda (2)"
