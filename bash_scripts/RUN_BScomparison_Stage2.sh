...
do
  ...
  do
    ...
    srun -N 1 -p gpu --gres=gpu:1 --mpi=pmi2 python /clusterdata/uqdlucha/scripts/deepReconPyTorch/train_stage2_WAE_iter.py --run_name WAE_Stage2_${SUBJ}_BS${BATCH_SIZE}_${RUN_NAME}_${RUN_TIME} --load_from ${LOAD_FROM} --pretrained_net ${STAGE_1_NET} --load_epoch ${STAGE_1_EPOCH} --disc_loss Both --WAE_loss Both --lambda_WAE 1 --batch_size ${BATCH_SIZE} --lr_enc 0.0001 --lr_disc 0.00005 --valid_shuffle True --latent_dims 512 --vox_res 1.8mm --set_size max --subject ${SUBJ} --ROI VC --num_workers 2 --epochs 85 --iters 20000 --dataset NSD --seed 277603 --message "Testing different BS"
